...
// Funciòn para calcular las ganancias después de realizar una orden de compra y venta para saber si 
// se va ganando o perdiendo
async function _calculateProfits() {
    const orders = store.get('orders') // se crea un array vacio llamado orders
    // se crea un array vacio llamado sold para filtrar e incluir las ordenes vendidas dentro del estado de ordenes
    const sold = orders.filter(order =>{ 
        return order.status === 'sold'
    })    
// 
    const totalSoldProfits = sold.length > 0 ?
    sold.map (order => order.profit).reduce((prev, next) => parseFloat(prev) + parseFloat(next)) : 0
...
function _logProfits(price){
    ...
    logColor (isGainerProfit == 1 ?
        ...
        `Global Profits (Inc. Fees): ${parseFloat(store.get('profits')).toFixed(4)} ${MARKET2}`)
...
async function _getFees ({commission, commissionAsset}){
    if (commissionAsset === MARKET2) return commission
    const price = await getprices(MARKET)
...
async function _buy(price, amount){
    if(parseFloat(store.get(`${MARKET2.toLowerCase()}_balance`)) >= BUY_ORDER_AMOUNT){
        ...
        var slFactor = process.env.STOP_LOSS_GIRD  * price / 100
...
// Funcion nueva no explicada videos de 5 meses
